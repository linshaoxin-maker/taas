"""
Calculate ROUGE score for Naive Baseline - Lead 3
"""
...
fin = open('data/cnndm/org_data/test.article', 'r', encoding='utf-8')
fgolden = open('data/cnndm/org_data/test.summary', 'r', encoding='utf-8')
dedup_rate = 0.7
trunc_len = 0
...
for line in fin:
    ...
    # modified_line = line.strip().replace('[X_ SEP]', '[X_SEP]').replace('X_SEp]', '[X_SEP]').replace('[X _SEP', '[X_SEP]').replace(' X_SE ', '')
    sentence_list = line.strip().split('<S_SEP>')
    for i in range(min(3, len(sentence_list))):
        sentence = fix_tokenization(sentence_list[i])
