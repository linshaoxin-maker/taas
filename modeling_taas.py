...
class BartEncoder(nn.Module):
    ...
    def forward(
        ...
        # with open(f"./output/cnndm/case_study/taas-topic-10/article.txt", "w") as f:
        #     f.write(tokenizer.decode(input_ids[0]))
        # import numpy as np
        # input_ids_cpu = input_ids.cpu()
        # your_file = open(f"./output/cnndm/case_study/taas-topic-10/input-id.txt", "w")
        # np.savetxt(your_file, input_ids_cpu.numpy())
        # your_file.close()
...
class BartModel(PretrainedBartModel):
    ...
    def forward(
        ...
        # here "encoder_outputs[0]" is the result from BART ENCODER - last layer
        # type(encoder_outputs): tuple
        # type(encoder_outputs[0]): tensor, encoder_outputs[0].shape: torch.Size([1, 1024, 1024])
        # last_hidden_state (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length, embedding dimensions)`):
        # Sequence of hidden-states at the output of the last layer of the model.
        # hidden_size – Dimensionality of the encoder layers and the pooler layer.

        # topic_p: kwargs['topic_p']
        # tm_loss: kwargs['tm_loss']

        # ----------------------------
        # Assign Topic-Weighted Attention
        # ----------------------------

        # calculate topic-token attention
        # a = softmax(h P^T)
        # softmax = nn.Softmax(dim=0)
        # topic_token_attention: torch.Size([1024, 50])

        # encoder_outputs[0] <-> h: num * embedding_dimension
        # num: number of tokens in the input sequence
        # topic_p <-> p: K * embedding_dimension
        # K: number of topics
        ...
        if input_ids != None:
            ...
            for batch_num in range(batch_size):
                ...
                # alpha_cpu = alpha.cpu()
                #
                # your_file = open(f'./output/cnndm/case_study/taas-topic-10/alpha-{batch_num}.txt', 'w')
                # np.savetxt(your_file, alpha_cpu.numpy())
                # your_file.close()
                # your_file = open(f'./output/cnndm/case_study/taas-topic-10/input-{batch_num}.txt', 'w')
                # np.savetxt(your_file, input_cpu.numpy())
                # your_file.close()

                # save output attentions
                # import numpy as np
                # attention_cpu = encoder_outputs['attentions'][-1][0][0].cpu()
                #
                # your_file = open(f'./output/cnndm/case_study/taas-topic-10/attention.txt', 'w')
...
class BartForConditionalGeneration(PretrainedBartModel):
    ...
    def forward(
        ...
        # enter Class BartModel() - forward() function
        outputs = self.model(
            input_ids,
            attention_mask=attention_mask,
            decoder_input_ids=decoder_input_ids,
